...
plt.savefig(f'{outdir}/scHiC_MajorType.exc.png',dpi=300)
plt.close()
...
with open('cell.exc.pkl', 'rb') as f:
    cell = pickle.load(f)
cell2cluster = dict(zip(cell.PDCLUSTER['cell'].unique(),np.arange(len(cell.PDCLUSTER['cell'].unique()))))
...
scRNAID2Type = { 0:'L2-3_CUX2', 1:'L4_RORB', 2:'L5-6_THEMIS', 3:'L5-6_TLE4'}
...
plt.tight_layout()
plt.savefig('/data/kfang/Yufan/inter_genes_table.exc.png',dpi=300)
plt.close()
# score
...
plt.xticks(rotation=60)
plt.tight_layout()
plt.savefig('/data/kfang/Yufan/inter_score_table..exc.png',dpi=300)
plt.close()