...
### scHi-C WTC11 and MCF7 data analysis
...
from cluster import cpu         ### load schicluter package
from mpl_toolkits.mplot3d import Axes3D
import random

os.chdir('/data/yufan/schic/public/ren4dn/wtc11mcf7')
os.getcwd()
...
celllabel1 = pd.DataFrame({'name':group1, 'sample':['MCF7'] * len(group1), 'path':path1})
...
celllabel5 = pd.DataFrame({'name':group5, 'sample':['WTC11'] * len(group5), 'path':path5})
...
celllabel6 = pd.DataFrame({'name':group6, 'sample':['WTC11'] * len(group6), 'path':path6})

celllabel = pd.concat([celllabel5, celllabel6, celllabel1], axis = 0)
celllabel.reset_index(drop = True, inplace = True)
celllabel.to_csv('cellLabel.txt',sep='\t',header=True,index=False)

group56 = group5 + group6

#network = group1 + group2 + group3
#network = group1 + group5 + group6
network = group56 + group1
...
plt.legend([p1, p2], ['WTC11', 'MCF7'], loc='upper right')
...
ax.view_init(18, 161)
#plt.legend([p1, p2, p3], ['MCF7', 'MCF7-T1M', 'MCF7-TamR'], loc='upper left')
plt.legend([p1, p2], ['WTC11', 'MCF7'], loc='upper left', fontsize=8)
...
###
clusterlabel = ['WTC11'] * cellnum1 + ['MCF7'] * cellnum2
pdcluster = pd.DataFrame({'sample': network, 'label': clusterlabel, 'cluster': yhat, 'PC1': X[:, 0], 'PC2': X[:, 1], 'PC3': X[:, 2]})

pdcluster.to_csv('wtcpdcluster.txt', index=False, header=True, sep='\t')

##########################
pdcluster = pd.read_csv('/data/yufan/schic/public/ren4dn/wtc11mcf7/wtcpdcluster.txt', header=0, sep='\t', skiprows=None)
...
clusterc9 = pdcluster[pdcluster.cluster==9]['sample']

clusterno = clusterc0
setlist = []
for i in clusterno:
	runpath = "/data/yufan/schic/public/ren4dn/hicpro/" + i.split('_')[0] + "/output/hic_results/matrix/"
	rundata = runpath + i.split('_')[1] + "/raw/" + str(res) + "/" + i.split('_')[1] + "_" + str(res)+ ".matrix"
	print("Reading " + rundata + "......")
	csvmat = pd.read_csv(rundata, header=None, sep="\t")
	csvmat.columns = ["bin1", "bin2", "binvalue"]
